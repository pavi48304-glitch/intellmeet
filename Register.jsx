...
import { Video, ShieldAlert, Lock, Mail, User, Loader, X } from 'lucide-react';
...
export const Register = ({ onNavigate }) => {
  ...
  const handleSSOSubmit = async (e) => {
    ...
    setTimeout(async () => {
      try {
        ...
        setValidationErr(err.message || `${ssoProvider} OAuth2 sign up failed.`);
        setOauthLoading(false);
      }
    }, 1200);
  };

  // Sample Avatar selections
  const avatarOptions = [
  ...
  return (
    <div className="min-h-screen flex w-full overflow-hidden bg-slate-950">
        ...
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center p-4">
          <div className="bg-[#121727] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <div className="flex flex-col items-center mb-6 mt-2">
              ...
              <h3 className="text-lg font-bold text-white">{ssoProvider} Sign Up</h3>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-black/30 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-cyan-500"
                  />
                </div>
              </div>
              <div>
      ...
      <div className="w-full max-w-md glass-panel rounded-2xl p-8 shadow-2xl relative z-10 glow-cyan border border-white/5">
        
        {/* Header */}
        <div className="flex flex-col items-center mb-6">
          <div className="w-14 h-14 bg-gradient-to-tr from-cyan-400 to-purple-600 rounded-xl flex items-center justify-center shadow-lg shadow-cyan-500/20 mb-3">
            <Video className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white mb-1">
            Sign up for <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Free</span>
          </h1>
          <p className="text-gray-400 text-sm">Join IntellMeet, the Next-Gen Workspace</p>
        </div>

        {/* Validation Errors */}
        {(validationErr || error) && (
          <div className="mb-5 bg-red-950/40 border border-red-500/30 rounded-xl p-4 flex items-start gap-3 text-red-200 text-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-white/10 bg-black/30 text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all text-sm"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-around items-center bg-black/20 border border-white/5 rounded-xl p-3">
              {avatarOptions.map((avUrl, index) => (
                <button
                  key={index}
                  type="button"
                  onClick={() => setSelectedAvatar(avUrl)}
                  className={`w-12 h-12 rounded-lg border-2 p-1.5 transition-all bg-slate-900 cursor-pointer ${
                    selectedAvatar === avUrl ? 'border-cyan-400 scale-110 shadow-lg shadow-cyan-400/20' : 'border-transparent hover:border-white/20'
                  }`}
                >
                  <img src={avUrl} alt={`Avatar option ${index + 1}`} className="w-full h-full" />
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-semibold rounded-xl transition-all shadow-lg hover:shadow-cyan-500/20 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 text-sm pt-3 pb-3"
          >
            {loading ? (
          ...
          <div className="grid grid-cols-3 gap-3">
            {['Microsoft', 'Apple', 'GitHub'].map(provider => (
              <button
                ...
                disabled={loading || oauthLoading}
                className="py-2.5 bg-black/40 hover:bg-black/60 border border-white/10 rounded-xl transition-all flex items-center justify-center cursor-pointer disabled:opacity-50 text-white hover:scale-105"
